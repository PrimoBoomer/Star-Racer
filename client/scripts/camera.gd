...
func _snap_to_car() -> void:
	self._cam_yaw     = self._car.global_rotation.y
	self._orb_yaw     = 0.0
	self._orb_pitch   = 0.0
...
func _tick_yaw(delta: float) -> void:
	var target_yaw := self._car.global_rotation.y

	if Input.is_action_pressed("Back View"):
		target_yaw += PI
	target_yaw += self._orb_yaw

	if Input.is_action_just_pressed("Back View") or Input.is_action_just_released("Back View"):
